...
/// Example: Tag 2 NWR    Tag 4 HBK   Tag 5 IGN     Tag 10 OKU     Tag 23 CLN\n
fn parse_activity_line(input: &str) -> IResult<&str, Vec<&str>> {
    map(
        many_till(delimited(space0, take_until("  "), space0), line_ending),
...
fn parse_day_entry(input: &str) -> IResult<&str, DayEntry> {
    map(
        tuple((
            ...
            parse_mood,
            parse_day_hour,
            many0(parse_activity_line),
            opt(parse_note_body),
        )),
        |(date, mood, day_hour, activities, note)| {
            let tags = activities.iter().flatten().map(|&s| s.to_owned()).collect();
